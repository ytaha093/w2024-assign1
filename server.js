...
app.get("/api/seasons", async (req,res) => {
  try {
    ...
    if(data.length === 0 ) res.json({error: "no results found"})
...
app.get("/api/circuits/season/:year", async (req,res) => {
  try {
    ...
    if(isNaN(req.params.year)) res.json({error: "string was entered instead of a number for year"})
    if(data.length === 0 ) res.json({error: "no results found"})
    if(error) res.send(error)
    res.send(data)
  } catch(error) {
...
app.get("/api/drivers/race/:raceId", async (req,res) => {
  try {
    ...
    if(isNaN(req.params.raceId)) res.json({error: "string was entered instead of a number for raceId"})
    if(data.length === 0 ) res.json({error: "no results found"})
    if(error) res.send(error)
    res.send(data)
  } catch(error) {
...
app.get("/api/races/season/:year/:round", async (req,res) => {
  try {
    ...
    if(isNaN(req.params.year) || isNaN(req.params.round)) res.json({error: "string was entered instead of a number for year or round"})
    if(data.length === 0 ) res.json({error: "no results found"})
    if(error) res.send(error)
    res.send(data)
  } catch(error) {
...
app.get("/api/races/circuits/:ref/season/:start/:end", async (req,res) => {
  try {
    ...
    if(isNaN(req.params.start) || isNaN(req.params.end)) res.json({error: "string was entered instead of a number for years"})
    if(req.params.start > req.params.end) res.json({error: "end year must come after start year"})
    if(data.length === 0 ) res.json({error: "no results found"})
    if(error) res.send(error)
    res.send(data)
  } catch(error) {
